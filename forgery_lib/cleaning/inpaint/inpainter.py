...
class Inpainter():
    def __init__(self, image, mask, message=False, patch_size=9,plot_progress=False):
    ...
    def _initialize_attributes(self):
        ...
        if len(self.image.shape) == 2:
            self.working_image = np.dstack((self.image,
                                            self.image, self.image))
        else:
            self.working_image = self.image[:, :, :3]
        self.working_mask = np.copy(self.mask)
        self.working_mask[self.working_mask>0] = 1
    ...
    def _find_source_patch(self, target_pixel):
        ...
        # Need to optimize the lines bellow
        # We could try doing it in parallel
        rgb_mask = self._to_rgb(self.working_mask)
        rgb_mask = 1 - Inpainter._patch_data(rgb_mask, target_patch)
        target_data = Inpainter._patch_data(lab_image, target_patch) * rgb_mask

        all_patches = [((y, y + patch_height-1),
                       (x, x + patch_width-1))
        ...
        def vectorize_function(source_patch):
            if Inpainter._patch_data(working_mask, source_patch).sum() != 0:
                return np.inf
            return Inpainter._calc_patch_difference(
                    lab_image,
                    target_patch,
                    target_data,
                    source_patch,
        ...
        differences = np.array([vectorize_function(patch)
                                for patch in all_patches])

        best_match_difference = np.argmin(differences)
        best_match = all_patches[best_match_difference]
    ...
    @staticmethod
    def _calc_patch_difference(image, target_patch, target_data,
                               source_patch, rgb_mask):
        source_data = Inpainter._patch_data(image, source_patch) * rgb_mask
    ...
    def _finished(self):
        ...
        if total != self.operation_status:
            self.operation_status = total
        # Method get stuck
        else:
            self.dilate_working_mask()
        if self.message:
            print(f'Processing Inpainting {(100*total):.2f}%',end="\r",flush=True)
        return remaining == 0

    def dilate_working_mask(self):
        kernel = np.ones((11,11),np.uint8)
        self.working_mask= cv2.dilate(self.working_mask,kernel,iterations = 2)
        self.working_mask[self.working_mask>0] = 1
