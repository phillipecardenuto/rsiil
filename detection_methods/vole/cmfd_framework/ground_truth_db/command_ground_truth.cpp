...
int GroundTruth::execute()
{
	...
	cv::Mat_<cv::Vec3b> original = cv::imread(config.orig_file, CV_LOAD_IMAGE_COLOR);
    if (original.empty()) { std::cerr << "error loading original image " << config.orig_file << std::endl; return 1; }


	cv::Mat_<cv::Vec3b> tampered = cv::imread(config.copy_file, CV_LOAD_IMAGE_COLOR);
	if (tampered.empty()) { std::cerr << "error loading image " << config.copy_file << std::endl; return 1; }

	cv::Mat_<cv::Vec3b> snippet =  cv::imread(config.snippet_file, CV_LOAD_IMAGE_COLOR);
	if (snippet.empty()) { std::cerr << "error loading image " << config.snippet_file << std::endl; return 1; }

	cv::Mat_<unsigned char> snippet_alpha = cv::imread(config.snippet_alpha_file, CV_LOAD_IMAGE_GRAYSCALE);
	if (snippet_alpha.empty()) { std::cerr << "error loading image " << config.snippet_alpha_file << std::endl; return 1; }

	cv::Mat_<unsigned char> full_snippet_alpha = cv::imread(config.full_snippet_alpha_file, CV_LOAD_IMAGE_GRAYSCALE); // grayscale
