...
void SpliceCore::splice(bool ground_truth_is_cmfd) {
	...
	for (int i = 0; i < (int) orig_snippets.size(); ++i) {
		...
		if (ground_truth_is_cmfd)
			splice(i, source_center_positions[i], true, ground_truth_is_cmfd );

		splice(i, insert_center_positions[i], false, ground_truth_is_cmfd );
	}
	...
	for (int i = 0; i < (int) orig_snippets.size(); ++i) {
		...
		int sx = insert_center_positions[i].x - (snippet_alpha[i].cols/2);
		int sy = insert_center_positions[i].y - (snippet_alpha[i].rows/2);
		int minx = std::max<int>(-sx, 0);
		int miny = std::max<int>(-sy, 0);
		int maxx = std::min<int>(alpha_mask.cols-sx, snippet_alpha[i].cols);
		int maxy = std::min<int>(alpha_mask.rows-sy, snippet_alpha[i].rows);
...
void SpliceCore::splice(int snippet_no, cv::Point src_pos, bool is_source, bool ground_truth_is_cmfd) {
	...
	// out of image bounds?
	// NOTE: CRASHES WHEN ALL COORDINATES ARE WAY OFF (e.g. because wrong, much smaller input image)
	bool alpha_changed_must_replay =
	//	cropBoundaryPixels( alpha, snippet, std::min<int>(alpha.cols, snippetLeftOffset.x), std::min<int>(alpha.rows, snippetLeftOffset.y),
	//		std::min<int>(alpha.cols, snippetRightOffset.x), std::min<int>(alpha.rows, snippetRightOffset.y));
...
//			if (alpha[y][x] < transparency_threshold) continue; // not sufficiently opaque? skip...
			...
			if (!is_source && ground_truth_is_cmfd) { // multiple copies from one source are ok and don't need to be validated
			// note that self-occlusion is possible, i.e. that a snippet shadows itself
				for (int i = snippet_no; i >= 0; --i) { // we check only if target regions killed source regions
					...
					cv::Point otherSnippetOffset(
						imgOffset.x                                               /* upper-left corner topmost-img */
						- (source_center_positions[i].x - (orig_alpha[i].cols/2)) /* upper-left corner other snippet */
						+ x,                                                      /* current position */
						imgOffset.y /* upper-left corner topmost-img */
						- (source_center_positions[i].y - (orig_alpha[i].rows/2)) /* upper-left corner other snippet */
						+ y /* current position */
					);
...
void SpliceCore::add_snippet_jpeg(int snippet_no, int quality) {
	...
	snippets[snippet_no] = cv::imdecode(wrap, CV_LOAD_IMAGE_COLOR);
}

void SpliceCore::add_global_cont_scale(double scale_factor) {
	cv::Mat dest, alpha_dest;
	cv::resize(img, dest, cv::Size(), scale_factor, scale_factor, cv::INTER_LINEAR);
	cv::resize(alpha_mask, alpha_dest, cv::Size(), scale_factor, scale_factor, cv::INTER_LINEAR);
	img = dest;
	alpha_mask = alpha_dest;
}

void SpliceCore::add_global_discrete_scale(double scale_factor) {
	cv::Mat dest, alpha_dest;
	cv::resize(img, dest, cv::Size(), scale_factor, scale_factor, cv::INTER_NEAREST);
	cv::resize(alpha_mask, alpha_dest, cv::Size(), scale_factor, scale_factor, cv::INTER_NEAREST);
	img = dest;
	alpha_mask = alpha_dest;
}
...
void SpliceCore::add_global_jpeg(int quality) {
	...
	cv::Mat_<unsigned char> wrap(buf);
	img = cv::imdecode(wrap, CV_LOAD_IMAGE_COLOR);
}
...
void SpliceCore::add_snippet_noise(int snippet_no, double stddev, bool blur) {
	...
	for (int y = 0; y < noise.rows; ++y) 
		for (int x = 0; x < noise.cols; ++x) 
			for (int c = 0; c < 3; ++c) 
				snippets[snippet_no][y][x][c] = (uchar)std::max<int>(0, std::min<int>(255, snippets[snippet_no][y][x][c] + noise[y][x][c]*255));
}
...
void SpliceCore::add_global_noise(double stddev, bool blur) {
	...
	for (int y = 0; y < noise.rows; ++y) 
		for (int x = 0; x < noise.cols; ++x) 
			for (int c = 0; c < 3; ++c) 
				img[y][x][c] = (uchar)std::max<int>(0, std::min<int>(255, img[y][x][c] + noise[y][x][c]*255));
}
