	...
	ind(N)
{
	...
	if (dim_in > 0) 
		dim = dim_in;
	std::cerr << "kdtree2: N, dim: " << N << ", " << dim << std::endl;
	build_tree();
...
//			if (spread < 0.000001) {
//				std::cout << "spread = " << spread << " (node->box[" << i << "].upper = " << node->box[i].upper << ", node->box[" << i << "].lower = " << node->box[i].lower << ")" << std::endl;
//			}
			if (spread>maxspread) {
				maxspread = spread;
				c=i; 
		...
		if (false) {
			...
			if (true) {
				...
				for (int k=l; k <= u; k++) {
					if (k < 0 || c < 0 || ind[k] < 0){
						std::cerr << "warning: kdtree2: sth wrong here: k= " << k << " c " << c << " ind[k] " << ind[k] << std::endl;
//						continue;
					}
